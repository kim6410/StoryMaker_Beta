  ...
  function renderList() {
    ...
    list.innerHTML = filtered.map((job) => {
      ...
      return `<article class="archive-card">
        ${thumb ? `<img class="archive-thumb" loading="lazy" src="${thumb}" alt="미리보기">` : '<div class="archive-thumb-placeholder">미리보기 없음</div>'}
        <div class="archive-main">
          <div class="card-head"><h3>${esc(job.title || 'Beta 제작')}<span class="status-pill">${esc(job.status || 'created')}</span></h3></div>
        ...
        <div class="archive-assets">
          ...
          <button type="button" class="detail-button" data-job="${esc(job.beta_job_id)}">상세</button>
        </div>
      </article>`;
    }).join('');
    ...
    try {
      ...
      detail.innerHTML = `<div class="detail-head"><div><div class="badge">BETA ARCHIVE DETAIL</div><h2>${esc(job.title || 'Beta 제작')}</h2><p>${esc(job.business?.name || '')} · ${esc(job.business?.region || '')} · ${esc(job.business?.service || '')}</p></div><button id="archive-detail-close" type="button">닫기</button></div>
        ...
        <div class="archive-sections">
          <details class="archive-section"><summary>업로드 이미지 ${images.length}장</summary><div class="archive-section-body"><div class="image-grid">${images.map((_, index) => `<a href="/beta-api/browser/jobs/${encodeURIComponent(jobId)}/image/${index + 1}" target="_blank"><img loading="lazy" src="/beta-api/browser/jobs/${encodeURIComponent(jobId)}/image/${index + 1}" alt="이미지 ${index + 1}"></a>`).join('') || '<div class="empty-mini">이미지가 없습니다.</div>'}</div></div></details>
          <details class="archive-section"><summary>팟캐스트 MP3</summary><div class="archive-section-body">${audioUrl ? `<audio controls src="${audioUrl}"></audio><a class="download-link" href="${audioUrl}" target="_blank">MP3 열기</a>` : '<div class="empty-mini">MP3가 없습니다.</div>'}</div></details>
          <details class="archive-section"><summary>자막 SRT</summary><div class="archive-section-body">${subtitleUrl ? `<a class="download-link" href="${subtitleUrl}" target="_blank">SRT 보기·다운로드</a>` : '<div class="empty-mini">SRT가 없습니다.</div>'}</div></details>
          <details class="archive-section"><summary>썸네일</summary><div class="archive-section-body">${thumbnailUrl ? `<a href="${thumbnailUrl}" target="_blank"><img class="thumbnail-preview" src="${thumbnailUrl}" alt="썸네일"></a>` : '<div class="empty-mini">썸네일이 없습니다.</div>'}</div></details>
          <details class="archive-section"><summary>최종 MP4</summary><div class="archive-section-body">${videoUrl ? `<video controls src="${videoUrl}"></video><a class="download-link" href="${videoUrl}" target="_blank">MP4 열기</a>` : '<div class="empty-mini">MP4가 없습니다.</div>'}</div></details>
        </div>`;
